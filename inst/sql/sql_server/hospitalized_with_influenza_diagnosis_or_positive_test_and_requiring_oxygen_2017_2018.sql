...
LEFT JOIN
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (2787821,2787819,2787822,2787820,4347665,2721315,2721316,2514577,4189560,4161978)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (2787821,2787819,2787822,2787820,4347665,2721315,2721316,2514577,4189560,4161978)
  and c.invalid_reason is null
...
WHERE (C.procedure_date >= DATEFROMPARTS(2017, 09, 01) and C.procedure_date <= DATEFROMPARTS(2018, 04, 01))
...
WHERE (C.observation_date >= DATEFROMPARTS(2017, 09, 01) and C.observation_date <= DATEFROMPARTS(2018, 04, 01))
...
WHERE (C.condition_start_date >= DATEFROMPARTS(2017, 09, 01) and C.condition_start_date <= DATEFROMPARTS(2018, 04, 01))
...
WHERE (C.drug_exposure_start_date >= DATEFROMPARTS(2017, 08, 31) and C.drug_exposure_start_date <= DATEFROMPARTS(2018, 03, 31))
...
WHERE (C.procedure_date >= DATEFROMPARTS(2017, 08, 31) and C.procedure_date <= DATEFROMPARTS(2018, 03, 31))
-- End Procedure Occurrence Criteria
